...
<html lang="en">
<head>
	<title>@yield('title') - {{ \\App\Setting::get("site_name") }}</title>
	...
	<div class="navbar navbar-{{ Auth::user()->customer->getColorScheme() == "white" ? "default" : "inverse" }} navbar-fixed-top">
		<div class="navbar-header">
			<a class="navbar-brand" href="{{ action('HomeController@index') }}">
				@if (\\App\Setting::get('site_logo_small'))
                    <img src="{{ action('SettingController@file', \\App\Setting::get('site_logo_small')) }}" alt="">
                @else
